...
#define GUI_TAG "GUI_TASK"
#define NOW_TAG "ESP_NOW_TASK"
...
static void esp_now_recv_callback(const esp_now_recv_info_t *recv_info, const uint8_t *data, int data_len)
{
    ...
    {
        ESP_LOGE(NOW_TAG, "Received invalid data length");
        return;
    }

    esp_now_data_t *received_data = (esp_now_data_t *)data;

    ESP_LOGI(NOW_TAG, "Received Data:");
    ESP_LOGI(NOW_TAG, "Battery 1 Voltage: %f", received_data->batt1_voltage);
    ESP_LOGI(NOW_TAG, "Battery 2 Voltage: %f", received_data->batt2_voltage);
    ESP_LOGI(NOW_TAG, "Total Voltage: %f", received_data->total_voltage);
...
void initialize_esp_now(void)
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(NOW_TAG, "Failed to initialize ESP-NOW: %s", esp_err_to_name(err));
        return;
    }
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(NOW_TAG, "Failed to register receive callback: %s", esp_err_to_name(err));
        return;
    }
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(NOW_TAG, "Failed to add peer: %s", esp_err_to_name(err));
        return;
    }

    ESP_LOGI(NOW_TAG, "ESP-NOW Initialized Successfully");
}
...
void app_main(void)
{
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE(GUI_TAG, "LCD initialization failed");
        return;
    }
    ...
    if (disp == NULL)
    {
        ESP_LOGE(GUI_TAG, "LVGL initialization failed");
        return;
    }
